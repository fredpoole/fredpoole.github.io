...
#Loading Data
When loading data into R you will often need to use a package for files other than .csv. If you have not already installed a package into your R environment, you'll need to run the following code in the console: `install.packages("readxl")` **(Don't forget to add the parentheses when installing a package...loading a library does not use parentheses)**. After you have installed it once, you can simply recall the package by running `library(readxl)`. Below are list of the packages that are needed for each file type. 
...
```{r}
library(readxl)
eGlossData <- read_excel("eGlossData.xlsx")
```
</br>
</br>
