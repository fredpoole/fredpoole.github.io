...
```{r}
df$Pre_cat <- ifelse(df$Per_Pre >= .78, "High","Low")
```
...
I like using `stargazer` because it prints out a formatted table. In the example below I have set `type` to text so that it will print out below the code, but you could change this to html or latex. 
```{r warning=FALSE}

stargazer(df,type="text",digits=2) #I used the digits code to limit how long the numbers are. 
```
